  ...
  const handleApplyFilters = () => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      if (Array.isArray(value) && value.length > 0) {
        params.set(key, value.join(',')); // Use set instead of append to avoid duplicate keys
      } else if (key === 'price') {
        params.set(key, value.join('-')); // Properly format the price range
      } else if (key === 'sort' && value) {
        params.set(key, value);
      }
    });

    setSearchParams(params); // Correctly update the search parameters
  };

  const handleClearFilters = () => {
    setFilters({ category: [], subCategory: [], gender: [], price: [0, 100000], sort: 'price-asc' });
    setSearchParams({}); // Clear search params in the URL
  };
